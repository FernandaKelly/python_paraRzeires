...
E você estar se perguntando: Fer, porquê você usou o **.astype** após aplicar o **pandas.to_mumeric**?
...
```{python}
df.replace([float("inf"), float("-inf")], pandas.NA, inplace=True)  # Substitui infinitos por NaN
colunas_float = ["ano", "orcamento", "receita", "receita_eua", "nota_imdb", "num_criticas_publico", "num_criticas_critica"]

df[colunas_float] = df[colunas_float].apply(pandas.to_numeric, errors="coerce").round(0).astype("Int64")  # Converte para número, mantendo NaN

df[colunas_float] = df[colunas_float].astype("Int64")  # Agora converte para Int64

df[colunas_float] = df[colunas_float].round(0).astype("Int64")  # Arredonda antes de converter
